...
async function loadPrints(name, lang) {
    ...
    // Cambiamos dir=asc por dir=desc
    const url = `https://api.scryfall.com/cards/search?q=${encodeURIComponent(`!"${name}" lang:${lang}`)}&unique=prints&order=released&dir=desc`;
    try {
...
async function generatePDF() {
    ...
    try {
        ...
        const totalPages = Math.ceil(deck.length / 9);
        setLog(t('log_pdf_done', deck.length, totalPages), 'ok');
    } catch (err) {
